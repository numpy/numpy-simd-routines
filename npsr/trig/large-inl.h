...
template <bool IS_COS, class V> HWY_API V LargeArg(V x) {
  using namespace hwy;
  ...
  VI i_table_idx = BitCast(di, Add(ShiftLeft<1>(u_exponent), u_exponent)); // × 2 + 1 = × 3
  ...
  VU u_integer_bit = Or(u_significand, Set(du, static_cast<TU>(1) << MantissaBits<T>()));
  ...
  constexpr int kQuotientShift = ExponentBits<T>() + 1; // 9 for F32, 12 for F64
  ...
  constexpr int kTotalShift = kQuotientShift + kFracLowShift + kFracMidShift + kFracHighShift;
  static_assert(kTotalShift == (kIsSingle ? 46 : 104), "Total shift must equal 2×mantissa_bits");
  ...
  VU u_exponent_part = Xor(u_sign_bit, BitCast(du, Set(d, static_cast<T>(1.0))));
  ...
  constexpr TU kFracHighMask = (static_cast<TU>(1) << (kFracHighShift - kFracLowShift)) - 1;
  ...
  VU u_shifted_sig_mid = Or(ShiftLeft<kFracHighShift>(u_frac_high_bits), u_frac_mid_bits);
  ...
  constexpr int kTableEntryBytesShift = kIsSingle ? 2 : 3;
  VI u_table_index = BitCast(di, ShiftLeft<kTableEntryBytesShift>(u_index));
  ...
  V sin_hi = GatherIndex(d, table_base + 1, u_table_index);
  V sin_lo = GatherIndex(d, table_base + 2, u_table_index);
  V deriv = GatherIndex(d, table_base + 3, u_table_index);
  ...
  V poly_correction = MulAdd(corr1, sin_poly, total_linear_error);
  
  V cos_correction = MulAdd(sin_hi, cos_poly, corr);
  V final_correction = Add(cos_correction, poly_correction);

  // Assemble final result: primary_value + all_corrections
